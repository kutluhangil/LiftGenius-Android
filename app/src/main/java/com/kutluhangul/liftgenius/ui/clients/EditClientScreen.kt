...
fun EditClientScreen(
    ...
    when {
        ...
        uiState.client == null -> ErrorState(
            ...
            client = requireNotNull(uiState.client),
...
private fun EditClientForm(
    ...
    var fullName by remember(client.id) { mutableStateOf(client.fullName) }
    ...
    var weight by remember(client.id) { mutableStateOf(client.weight?.toString().orEmpty()) }
    var height by remember(client.id) { mutableStateOf(client.height?.toString().orEmpty()) }
    var notes by remember(client.id) { mutableStateOf(client.notes.orEmpty()) }
    var goal by remember(client.id) { mutableStateOf(client.goal) }
    var status by remember(client.id) { mutableStateOf(client.status) }
    ...
    Box(Modifier.fillMaxSize(), contentAlignment = Alignment.TopCenter) {
        Column(
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Text(
                    text = stringResource(R.string.client_edit_title),
            ...
            FlowRow(horizontalArrangement = Arrangement.spacedBy(Spacing.sm)) {
                ClientStatus.entries.forEach { option ->
                    FilterChip(
            ...
            GradientButton(
                ...
                onClick = { onSave(fullName, phone, email, goal, status, weight, height, notes) },
                loading = isSaving,
                modifier = Modifier.fillMaxWidth(),
            )
            Spacer(Modifier.height(Spacing.xxl))
